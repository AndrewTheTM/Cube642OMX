...
void TPPMatrix::openFile(char *fileName)
{
	...
	// We're looping for MAX_DLL_ATTEMPTS because Citilabs DLL can timeout
...
void TPPMatrix::readTableNames()
{
    ...
    if(c>0){ //Keep from trying to write a null string to the buffer, which will screw stuff up. Incidentally, this was done while debugging a matrix that HAD A NAME!!!)
